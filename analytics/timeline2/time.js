        ...
        const addToMap = (arr) => {
            ...
            arr.forEach(name => {
                if (map[name] === undefined) {
                    map[name] = {}
                    map[name][numQuotes] = 1
                    for (let i = 1; i < numQuotes; i++) {
                        map[name][i] = 0
                    }
                }
                else {
                    let cur = map[name][numQuotes - 1]
                    map[name][numQuotes] = (cur + 1)
                }
            })
            Object.keys(map).forEach(x => {
                if (map[x][numQuotes] === undefined) {
                    map[x][numQuotes] = map[x][numQuotes - 1]
                }
            })
            if (Object.keys(map).length > 20) {
                let sortedNames = Object.keys(map).sort((a, b) => {
                    return (map[b][numQuotes] - map[a][numQuotes])
                })
            }
        }
...
(async () => {
    ...
    chart = Highcharts.chart('container', {
        ...
        title: {
            text: 'Number of Quotes over time',
            align: 'left'
        },
        subtitle: {
            useHTML: true,
